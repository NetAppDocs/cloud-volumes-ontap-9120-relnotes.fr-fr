...
| Capacité système maximale (disques + stockage objet) ^1^ | 500 Gio | 2 Tio | 10 Tio | 368 Tio ^2^ | 368 Tio par licence ^2^ | 2 Pio ^2^ 
...
. Pour certaines configurations, les limites de disque vous empêchent d'atteindre la limite de capacité en utilisant uniquement des disques. Dans ces cas, vous pouvez atteindre la limite de capacité de https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/concept-data-tiering.html["tiering des données inactives vers le stockage objet"^]. Pour plus d'informations sur les limites de disque, reportez-vous link:reference-limits-aws.html["limites de stockage"]à la section .
